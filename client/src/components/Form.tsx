...
const Form = ({ conditionalValue }: FormProps) => {
  const [isRegister, setIsRegister] = useState(conditionalValue);
    ...
    try {
      if (isRegister) {
        const { data } = await createUser({
          variables: { input: { username, email, password, isAdmin: false } },
        });
          ...
          <div className="bg-white px-6 py-12 shadow-sm sm:rounded-lg sm:px-12">
            ...
            <div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-4">
                <a
                  href="#"
                  className="shadow-xs flex w-full items-center justify-center gap-3 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus-visible:ring-transparent"
                >
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
